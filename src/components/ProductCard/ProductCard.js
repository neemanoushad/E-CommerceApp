...
const ProductCard=({products=[]})=> {
  return (
    <section className="text-white body-font bg-customColor ">
    ...
    <div className="flex flex-wrap -m-4">
        {
            products.map((product)=>{
                ...
                <div className="mt-4">
                  <h3 className="text-white text-xs tracking-widest title-font mb-1 uppercase">{category}</h3>
                  <h2 className="text-white title-font text-lg font-medium">{title}</h2>
                  <p className="mt-1 text-sm font-semibold">Rs.{price}</p>
                </div>
              </Link>
              )
